...
public class GameOfLifeGUI implements ActionListener {
	...
	public void initField() {
		...
		for (int row = 0; row < grid.getRows(); row++) {
			for (int col = 0; col < grid.getCols(); col++) {
                ...
                //JButton changed to MyButton

				
				MyButton b = new MyButton();
				b.setActionCommand("Button "+row+" "+col);
				Cell c = grid.getCell(row, col);
				c.registerObserver(b); //Button wird zum Cell-"array" hinzugefügt
		...
		textFieldRow.setText(grid.getRows()+"");
		textFieldColumn.setText(grid.getCols()+"");
		cellPanel.updateUI();
		frame.pack();
		frame.setVisible(true);
	}
	...
	private void loadFile() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			grid.loadGrid(file);
			initField();
			labelGeneration.setText(GENERATION_PREFIX + 0);
		} else {
	...
	void nextGeneration() {
		...
		if (grid.checkStable()){
			...
			JDialog dialog = pane.createDialog(frame,"Game of Life");
			JOptionPane.showMessageDialog(dialog, "Stable after "+grid.getGeneration()+" generations" ,"Info",JOptionPane.INFORMATION_MESSAGE);	
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (ACTION_COMMAND_CLEAR.equals(sAction)) {
			...
			bExit = true;
			frame.dispose();
			System.out.println("Game of Life over and out");
			System.exit(0);
			...
			if (sAction.startsWith("Button")){
				try {
					String[] button = sAction.split(" ");
					int x = Integer.valueOf(button[1]);
					int y = Integer.valueOf(button[2]);
